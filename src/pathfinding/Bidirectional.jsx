const BidirectionaAlgo = ({
  ...
  const getNeighbours = (node) => {
    ...
    for (let i = 0; i < directions.length; i++) {
      if (
        !(
          directions[i][0] < 0 ||
          directions[i][0] >= numCols ||
          directions[i][1] < 0 ||
          directions[i][1] >= numRows ||
          GridValue([directions[i][0], directions[i][1]]) !== 0
        )
      ) {
  ...
  if (!curStartNode || !curEndNode) {
    ...
    const path = findPath(curStartNode, curEndNode, delay);
